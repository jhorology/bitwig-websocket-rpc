...
class BitwigClient extends EventClient {
  ...
  async connect(arg) {
    ...
    do {
      try {
        ...
        const e = await this._promise(events, false, opts.timeout)
        switch (e.event) {
          case '_accepted':
            this._urlQuery = undefined
            this._options.password = opts.password
            connected = true
            break
          case '_authChallenge':
            this._urlQuery = this._generateChallengeResponse(
              JSON.parse(e.params),
              opts.password,
              1
            )
   ...
   * @param {boolean=true} merge - true if settings are diffrence from current configuration.
   * false if settings are diffrence from default configuration.
   * @return {Promise<object>} - present configuration after applying this method.
   */
  async config(settings, merge = true, reconnectionDelay = 300) {
    let config
    if (settings === null) {
      // reset
      config = [null]
      ...
      const fulFilled = Object.keys(settings).every(
        key => settings[key] === config[key]
      )
      if (fulFilled) {
        return config
      }
    ...
    // server may be not yet started, retry 3 times
  ...
  /**
   * Generate challenge response as url query params.
   */
  _generateChallengeResponse(challenge, passwd, nc) {
    const username = 'bitwig',
      ...
      a2 = md5(`:${uri}`),
      digest = md5(
        `${a1}:${challenge.nonce}:${strNc}:${cnonce}:${challenge.qop}:${a2}`
      )
    return (
      '/auth?' +
      `username=${username}` +
