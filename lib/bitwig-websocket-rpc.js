 ...
 * @return {Promise} resolve present configuration after applying this function.
 */
module.exports = async (url, config, merge) => {
  const ws = await bwsOpen(url),
        currentConfig = await ws.call('rpc.config', null, 5000),
  ...
  const newUrl = settings.webSocketPort
    ? url.replace(/(.*:)([0-9]+)$/, '$1' + settings.webSocketPort)
    : url,
        newWs = await bwsOpen(newUrl),
        newConfig = await newWs.call('rpc.config', null, 5000)
  newWs.close()
  return newConfig
}
