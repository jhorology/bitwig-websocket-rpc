    ...
    props: [
      {
        prop: 'notification',
        required: true,
        assert: v => v && typeof v === 'string'
      },
      {
        prop: 'params',
        required: false,
        assert: v => v && (typeof Array.isArray(v) || typeof v === 'object')
      }
    ]
  },
  {
...
class Client extends EventEmitter {
  ...
  get readyStateAsString() {
    ...
    return state >= 0 && state < 4
      ? ['0: CONNECTING', '1: OPEN', '2: CLOSING', '3: CLOSED'][state]
      : '' + state + ': unknwon state'
  }
  ...
  async connect(arg) {
    ...
    if (!this.isClosed) {
      throw new ClientError(
        'Could not connect to server: Illegal ready state.',
        'TRANSPORT',
        undefined,
        {
    ...
    do {
      try {
        ...
        const e = await this._promise(['open', 'close'], false, opts.timeout)
        switch (e.event) {
          case 'open':
            connected = true
            break
        ...
        if (this._ws && (this.isOpen || this.isConnecting)) {
          this._close()
        }
        if (opts.retry) {
          this._trace(
            'connect() attempting to reconnect. remaining retry count:',
            opts.retry,
            'error caused by:',
            err
          )
    ...
    } while ((opts.retry >= 0 || infinite) && !this._closedBySelf && !connected)
  }
  ...
  batch(builder, timeout = this._options.responseTimeout) {
    return new Promise((resolve, reject) => {
      ...
      if (promises.length === 0) {
        ...
        promises[0].then(result => resolve([result])).catch(reject)
      } else if (promises.length > 1) {
  ...
  // safe close
  _close(code = 1000, reason = undefined) {
    try {
      this._ws.close(code, reason)
    } catch (err) {
      // ignore
      this._trace('_close() err:', err)
    } finally {
      this._onDestroy()
    }
  }
  ...
  _onInit() {
    ...
    this._ws = new WebSocket(
      this._url + (this._urlQuery ? this._urlQuery : ''),
      this._protocols
    )
  ...
  _onClose(e) {
    this._trace('WebSocket.onclose() code:', e)
    this.emit('close', e)
    this._onDestroy()
  }
  ...
  _onMessage(e) {
    ...
    try {
      ...
      this.emit(
        'error',
        new ClientError('JSON parse error.', 'JSON-RPC', err, e.data)
      )
      return
    }
   ...
   * @param {boolean} [rejectOnError = true] - enable reject on error.
   * @param {number} [timeout = 3000] - the timeout millis.
   *   0 = infinite wait until match params.
   * @return {Promise<ResolvedEvent>}
   * @typedef {Object} ResolvedEvent
   * @property {string} event - The event name.
   * @property {Array|object} params - The event params.
   */
  _promise(
    event,
    rejectOnError = true,
    ...
    this._trace('_promise() waiting for event:', events, 'timeout:', timeout)
    return new Promise((resolve, reject) => {
      ...
      onEvents = events.map(e => params => {
        clean()
        if (rejectOnError && e === 'error') {
          reject(
            new ClientError(
              `WebSocket Error: while waiting event [${events}].`,
              'TRANSPORT',
              params
            )
          )
      ...
      if (timeout) {
        timerId = setTimeout(() => {
          ...
          reject(
            new ClientError(
              `Timeout Error: while waiting event [${events}].`,
              'TRANSPORT'
            )
          )
        }, timeout)
   ...
   */
  _debug(...data) {
    this._options.debugLog && this._options.debugLog(data)
  }
   ...
   */
  _trace(...data) {
    this._options.traceLog && this._options.traceLog(data)
  }
  ...
  _createRequest(method, params, notify) {
    this._options.chkArgs && this._chkArgs(method, params)
  ...
  _processResponse(response) {
    ...
    let msgDef = rxMessages.find(d => response[d.id] !== undefined),
      msg
